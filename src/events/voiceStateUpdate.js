...
const { addXp, getRank, xpForLevel } = require("../utils/levelSystem");
const { getActiveMultiplier, grantLevelRewards } = require("../utils/levelRewards");
...
async function handleVoiceXp(client, oldState, newState) {
  ...
  const hasSession = voiceSessions.has(key);
  ...
  if (wasInVoice && isInVoice) {
    // If the bot restarted (or cache was lost) while the member was already in voice,
    // recreate a session so XP can still be awarded on next leave.
    if (!hasSession) {
      voiceSessions.set(key, Date.now());
    }
    return;
  ...
  if (wasInVoice && !isInVoice) {
    ...
    const { getCachedMultiplier, queueXp } = require("../utils/xpBuffer");
    const baseXp = Math.min(300, minutes * 5);
    const multiplier = await getCachedMultiplier(db, guild.id, member.id);
    const gainedXp = Math.max(1, Math.floor(baseXp * multiplier));
    
    queueXp(guild.id, member.id, gainedXp);

    client
      .getLogger()
      ?.send(
        `[LEVEL] +${gainedXp} XP vocal en file d'attente pour ${member.user.tag} (${minutes} min, x${multiplier})`,
        "DEBUG",
      );
  }
}
...
const VoiceStateUpdate = {
  async executeHandler(client, oldState, newState) {
    ...
    if (guild && member) {
      try {
        await handleVoiceXp(client, oldState, newState);
      } catch (err) {
        client.getLogger()?.send(`[LEVEL] Erreur XP vocal: ${err.message}`, "ERROR");
      }
    }
