...
const messageCreate = {
  async executeHandler(client, message) {
    ...
    try {
      ...
      const multiplier = await getCachedMultiplier(db, message.guild.id, message.author.id);
      const gainedXp = Math.max(1, Math.floor(baseXp * multiplier));
      
      queueXp(message.guild.id, message.author.id, gainedXp);
    } catch (error) {
      safeSend(client.getLogger && client.getLogger(), `[LEVEL] Erreur queue XP: ${error.message}`, "ERROR");
    }
  },
