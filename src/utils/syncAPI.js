  ...
  async _fetchUserDataUncached(userId) {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        this.logger?.send(
          `[API] Requete vers ${url} (membre: ${memberLabel}, tentative ${attempts + 1}/${maxAttempts})`,
          "DEBUG",
        );
        ...
        for (const member of members.values()) {
          ...
          try {
            ...
            if (data) {
              ...
              syncSuccess++;
              this.logger?.send(
                `[AUTO_SYNC] [OK] ${member.user?.username || member.id} (${member.id})`,
                "DEBUG",
              );
              ...
              syncFailed++;
              this.logger?.send(
                `[AUTO_SYNC] [ERR] ${member.user?.username || member.id} (${member.id}) - ${error || "Erreur inconnue"}`,
                "WARN",
              );
            }
          } catch (e) {
