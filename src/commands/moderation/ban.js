...
const ban = {
  ...
  async executeCommand(client, interaction) {
    ...
    try {
      ...
      try {
        await getDatabase()("ban_table").insert({
          infraction_id: `ban_${member.user.id}_${Date.now()}`,
          server_id: guild.id,
          member_id: member.user.id,
          ban_date: dayjs().format("DD/MM/YYYY a HH:mm:ss"),
          ban_reason: reason,
          moderator: interaction.user.id,
        });
        client.getLogger().send(`Ban enregistre pour ${member.user.tag}`, "NOTIF");
      } catch (dbError) {
